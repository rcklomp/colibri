#!/bin/bash
# q1_chain.sh -- Q1: DeltaNet's serial tail (qwen38-vk).
#
...
#
# Differences from q3_chain.sh, both Q1-specific:
#   - the microbenchmark is rome_dnbench.c, not rome_cpubench.c: what this item
#     moves is three scalar loops, not an FP8 matmul. It runs its own
#     bit-identity oracle before it times anything.
#   - the summary prints the dn-* sub-rows, which is the whole point of step 0.
#
...
set -u
OUT=~/bench/q1_out; mkdir -p "$OUT"
SRC=~/src/colibri
WT=~/src/colibri-q1
BRANCH=${BRANCH:-perf/q1-deltanet-serial-tail}
QSNAP=~/models/Qwen3.8-Flash-Next-FP8
PRIS=~/bench/qwen38-vk.q1base
...
rig_lock_take "q1-chain" || { echo "rig busy"; exit 3; }
...
# rome_dnbench.c: the three serial loops at the engine's real shapes, with its
# own bit-identity oracle (36 layers x 64 tokens, ring state carried) run before
# any timing. No engine up, lock held, 8 threads pinned.
if [ "${SKIP_MICRO:-0}" != 1 ]; then
  if gcc -O3 -march=native -fopenmp -o /tmp/q1_dnbench \
        "$WT/tools/hot-expert/rome_dnbench.c" -lm > "$OUT/dnbench-build.log" 2>&1; then
    log "--- rome_dnbench (8 threads)"
    OMP_NUM_THREADS=8 OMP_PLACES=cores OMP_PROC_BIND=close /tmp/q1_dnbench 2>&1 \
      | tee "$OUT/dnbench.log"
  else
    log "WARNING: dnbench did not build"; tail -20 "$OUT/dnbench-build.log"
  fi
fi
...
resid(){  # $1 label  $2 assert|report
  fincore --bytes --output FILE,SIZE,RES "$QSNAP"/*.safetensors > /tmp/q1_fincore.txt
  python3 -c "
PAGE=4096; tot=res=0; short=0; n=0
for line in open('/tmp/q1_fincore.txt').read().splitlines()[1:]:
    f=line.split()
    if len(f)<3: continue
...
engine(){   # $1 bin  $2 tag  $3 n_new  $4 timers(0|1)  $5 extra env ("-" for none)
  cp -f "$HIST_SRC" /tmp/q1_hist.bin
  warm; resid "$2-pre" assert || { log "ABORT before $2"; exit 1; }
  log "--- run $2 n_new=$3 timers=$4 extra=${5:--}"
  env SNAP="$QSNAP" COLI_USAGE=/tmp/q1_hist.bin N_NEW="$3" \
      $( [ "$4" = 1 ] && echo COLI_TIMERS=1 ) \
      $( [ "${5:--}" != "-" ] && echo "$5" ) \
      DUMP="/tmp/q1_$2.f32" NOSTREAM=1 \
...
oracle(){  # $1 tag-a  $2 tag-b
  local L T a b
  if [ ! -s "/tmp/q1_$1.f32" ] || [ ! -s "/tmp/q1_$2.f32" ]; then L="MISSING(no dump written)"
  elif cmp -s "/tmp/q1_$1.f32" "/tmp/q1_$2.f32"; then L="IDENTICAL ($(stat -c%s "/tmp/q1_$1.f32") bytes)"
  else L="DIFFER -- first byte $(cmp "/tmp/q1_$1.f32" "/tmp/q1_$2.f32" 2>&1 | head -1)"; fi
...
for k in p:
    ...
    print(line)
# rows only the candidate has (the dn-* sub-timers, on a pristine without them)
seen=set()
for a in arms:
    for k in cands[a]:
...
for k in ("step","deltanet","dn-proj","dn-conv","dn-qknorm","dn-recur","dn-gnorm","dn-rest","moe","gr-read"):
