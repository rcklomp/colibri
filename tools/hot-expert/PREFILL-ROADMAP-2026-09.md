# Prefill / TTFT roadmap — GLM-5.3 on rome (opened 2026-09-06, rev 10, 2026-09-07 06:40)
...
| **P7** | Checkpoint the stable prefix (tools + base system) **and pin the per-turn context block** — spec `P7-PREFIX-CKPT-SPEC-2026-09.md` (rev 10) | engine: snapshot the slot's session (11 DSA layers' latent/ikeys/igates rows + 34 KDA states/windows, 334 MB at 5 400 tokens) at the prefix boundary — found from the gateway's 8th SUBMIT field or the LCP of successive fresh prompts, the `deepseek_v4.c` `v4_ckpt_*` pattern — and restore it when a new prompt strictly extends a stored prefix; persisted under `<SNAP>/.coli_ckpt` so a gateway restart is warm. Gateway: `COLI_PREFIX_PIN=1` keeps Open WebUI's `<memory_context>` block byte-identical across a conversation's turns (measured 2026-09-07: with ≥ 2 memories the block re-ranks every turn and turn 2 reuses 0) | turn 1 of every new conversation with the tool block: **876 s → ~10 s**; turn 2+ with memory on: full re-prefill → ~2 s; first request after a restart: warm | 2–4 days | Opus (spec: Fable, done) | `p7_gate.sh` exits 0: `prefill_gate.sh` neutral with checkpoints off; `ttft_serve.py --prefix-ckpt --oracle` (conversation 2 REUSE ≥ len(S) − 32, ttft < 0.25×, greedy text identical over 128 tokens, first-token logits cosine ≥ 1−1e-4); `tworeq` at 4 slots identical at both KDA knobs; `--pin-block` PASS; then the same two checks through the live gateway with `CKPT hit` in the server log |
