...
10. **Track Q** has its baseline (§Q-REBASE), its profile (§Q-PROFILE) and its
   ...
   **Q1 is merged (2026-09-12, arbitration in record §Q1): bit-identical,
   ...
   was over-counted.** **Q2 is next, and its own bucket is the same kind of
   estimate — its step-0 sub-timer re-derives its gate before anything is
   built (the rule above the Track Q table).** Then **Q10** (the recurrence's
   state traffic, out of Q1's sub-timers; step 0 decides whether it is a
   bandwidth or a latency item). Then
