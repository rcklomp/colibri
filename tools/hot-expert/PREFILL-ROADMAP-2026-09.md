# Prefill / TTFT roadmap — GLM-5.3 on rome (opened 2026-09-06, rev 8, 2026-09-07 00:xx)
...
| **P6** ⏳ | Make reuse survive Open WebUI — step 1 done (§P6 in the record: the single slot; 4 slots + CPU recurrence → turn 2 in 3.2 s under the harness). **Blocked on the gateway**: `family_registry` caps glm53 at 1 KV slot, `serve()` refuses `--kv-slots 4` (service restored at 1 slot, `COLI_KDA_GPU=2`). Remaining: (2) raise the family limit for glm53 **together with P6b** (per-slot KDA device state — the limit and the single-device-state hazard are the same fact), gate with `tworeq` per slot and the side-request case through the live gateway (`ttft_serve.py --url … --multiturn --side-request`); (3) confirm on the user's real Open WebUI chats from the server log's REUSE lines | (1) read the server log's REUSE lines from a real Open WebUI two-turn chat and name what broke the match (side requests on the single slot; per-turn system-prompt changes); (2) `--kv-slots N` on the gateway — `conversation_cache_slot` already routes turns by system+first-user hash — **but** with `COLI_KDA_GPU=2` the device holds one KDA state per layer, re-seeded on every `session_open`, so either serve with the CPU recurrence (−11.7 % decode) or give each slot its own device state / `kda_sync`+`kda_upload` on slot switch; (3) keep the rendered prefix stable per turn on the gateway side | turn 2+ from Open WebUI: **minutes → seconds** (what P1 measured, delivered on the user's path) | 2–5 days | Opus; Fable for the per-slot device-state decision | a two-turn chat from Open WebUI itself shows `REUSE` ≫ 0 on turn 2 in the server log, with a title-generation request in between; `tworeq.py` identical across 3 requests in every slot |
