...
| G12 | **IN PROGRESS 09-05 — stages 1 and 2a done (record §G12 stage 1 + 2a), chain not built.** Shader `kda_step.comp` validated against `coli_kda_step` over 4000 consecutive steps: output drift 2.85e-06 max, and the state error **saturates rather than compounds** (lower at 4000 steps than at 100). `COLI_KDA_GPU=1` now runs the recurrence on dev0: **step 0.378 → 0.168 ms/call (2.25×) even while still paying an extra round trip**, `kda` 1.574 → 1.191 ms/call = −13.0 ms/token. `teacher_forcing` identical at 42/210/630/1260 positions and greedy text identical through 128 decode tokens; logit cosine 1.0 at 42 positions falling to 0.99992 at 1260 — token selection unaffected, values drift at long context. Spec's LDS plan was wrong and was not followed (the tile *is* the whole 64 KB budget); state streams from VRAM and the per-thread accumulators are registers | §RP1-CORRECTION: KDA 53.3 ms/token, largest; 35.0 of it GPU submits | **−13.0 ms/token banked**; chain projects `kda` ~0.90 ms/call from measurement | 2 of 3 commits | **Opus** | go/no-go `kda` < 1.0 ms/call: **at 1.19, not yet met** — the chain is what closes it |
