...
its counter into a token (see the §Q3 note). **Merged 2026-09-12 by
...
the shared bucket, which the measurement meets at 54%.
...
of its estimated −8 to −13 as well, record §Q1 — and Q2's leg is still an
estimate of the same arithmetic-subtraction kind that both of the measured ones
came in under, so read the top of that range); then the int8 branch (Q7-cpu) to **~103–123**, or
...
10. **Track Q** has its baseline (§Q-REBASE), its profile (§Q-PROFILE) and its
   ...
   **Q1 is built and measured (2026-09-12, record §Q1) and is AWAITING A FABLE
   REVIEW on `perf/q1-deltanet-serial-tail`, not merged: bit-identical,
   `deltanet` 70.22 → 63.87 and the token 185.86 → 179.84 (−6.02), but its
   `deltanet ≤ 60` leg is unreachable — 62.33 ms/token of `deltanet` is the
   ...
   **Q2 is next, and its own bucket is the same kind of estimate — its step-0
   sub-timer decides what it is worth before anything is built.** Then
