...
absolute. Q2's "~9.9 (est.)" is the first row this binds. **Added at Q7's
arbitration (2026-09-12, record §Q7 `### Arbitration`): a step-0
microbenchmark that predicts an in-engine bucket for a GPU dispatch replays
the engine's own inter-submit gaps (the duty-cycle case in `rome_vkbench`),
≥ 5 repeats, and the per-step 1.25× is taken against that figure** — Q7 step
0's back-to-back 19.6 became 26.98 in the engine for a reason that was in
neither the kernel nor the threshold, and the item's "half" floor sits on the
arm (the sum of the steps that share one mechanism), not on each step.
...
| **Q5** | **DONE 2026-09-15 (Opus, `5504e51`) — bit-identical, the counter moves, the token does NOT, exactly as this row predicted.** Gate, all three legs: `teacher_forcing` IDENTICAL and logits BIT-IDENTICAL; `cpu-experts` **5.344/5.374 → 5.010/5.037 ms/token (−6.3%, arms non-overlapping)**, against a ≤13.5 threshold that was already met at 5.36 before the change; and the token did not move — tok/s base 2.91/3.13 vs cand 3.06/3.12, overlapping, with base's best run the fastest of all four (step 172.99/172.83 vs 175.61/175.81 ms/fwd, vk-take 13.40/13.21 vs 13.79/13.74). **Arms were interleaved base,cand,cand,base because a first uninterleaved pair read +11% that was pure cache warmth** — the third time that confound appeared on this rig in one day. Implementation note for the next reader: the obvious reuse does NOT work — `q38_dense_matmul_multi` is BF16-only and the routed experts are FP8, so `quant.h` gained `matmul_fp8_inregion` (the body of `matmul_fp8` with no region of its own) and the fused path applies only when all three weights are plain FP8, falling back otherwise so QP's simulation knobs are untouched. Previous row text follows. **RE-COSTED 2026-09-15 (source check, nothing run): the existing fused helper does NOT cover this path.** `q38_dense_matmul_multi` fuses several projections of the same input into one region and is the obvious thing to reuse — but it is **BF16-only** by construction ("Non-BF16 members fall back to the per-matrix path"), and the routed experts are FP8 through `matmul_fp8`. So this item is not "call the existing helper"; it needs a fused FP8 path written, or `matmul_fp8` restructured to take several weight sets. The half-day estimate below assumed the former. Its expected value is unchanged and still ~0 on the token (see the row's own reasoning). Previous row text follows. **Fuse the CPU expert's three matmuls into one OpenMP region.** `q38_moe_decode`'s CPU share calls `q38_weight_matmul` three times per expert — **3 fork/joins × 92 CPU experts = 276 OMP regions per token**. This is §G11 part 1, which was bit-identical on GLM. **Re-scoped from the draft: it is worth ~0 on the fresh-process token today**, because the CPU share runs *inside* the GPU gap and the GPU is the longer side (13.0 ms of idle remained after it before Q3; **8.4 ms after it**, measured as `vk-take` — record §Q3 — but unevenly distributed across layers: Q3's uniform 0.15 ms/layer of added CPU work was absorbed at only 54%, which makes this row's "worth ~0 on the token" firmer, not weaker); making the hidden side faster moves the counter, not the token. Its value is (i) slack in the gap, which Q7-gpu's eviction spends, and (ii) the rotating regime, where more experts miss the tier and the CPU side can be the longer one | cpu-experts **15.4 ms/token (8.1%)**, hidden | `rome_cpubench.c`: the same ten experts cost **1.602 ms/token-layer through the engine's loop and 1.387 fused — 1.16×**. Isolated and in-engine already agree to 4% (0.16 vs 0.167 ms/expert), so this is not an attenuation gamble | **−2.1 ms/token on the counter; 0 to −2 on the token** | half a day | Sonnet | **bit-identical** (§G11 part 1 was); `[OPTIME] cpu-experts` ≤ 13.5 ms/token; **report `step` and `vk-take` beside it and say plainly if the token did not move** — the counter is the gate, the token is the honest footnote |
| **Q4** | **ARBITRATION EXECUTED 2026-09-12 (Sonnet, record §Q4 `### Arbitration executed`): the four legs were run — knob stays OPT-IN, default 0, unchanged.** Two independent, unconditional leg failures, not one: **(leg 2) 2 of the 3 flips have a pristine top1/top2 margin above the 0.10 line** (0.248102 and 0.140074 at positions 714 and 852; only position 708 at 0.053267 is a genuine near-tie) — every flip IS a clean top1/top2 swap (the candidate always picks the pristine's exact runner-up, no cascade), but the margin bar is not met on 2 of 3; **(leg 3) routing differs on the long prompt**, 1077 of 57 600 `ROUTE_TRACE` rows (mostly low-rank tie-order swaps among an expert's 7th–10th pick). All three flips sit inside induction copies (positions 708/714/852, none in the first copy or at a copy boundary). The arbitration's router-exclusion retry applies only when routing is the *sole* failure; it is not here (leg 2 fails independently), so the narrower rebuild was not run — it cannot change the outcome. **Leg 4 (the natural, non-repeated prompt) passed by a wide margin**: `tools/hot-expert/MEASURING.md` verbatim tokenizes to 1558 tokens, `teacher_forcing` **0 of 1558 differ**, the 128-token greedy continuation is byte-identical, cosine **1.000000000** (relL2 1.2020e-06) — the cleanest result either track has produced on a reassociating change, and it pins the original 3-of-1200 fragility to the repeated-copy/induction regime specifically (§G14's mechanism), not to long real text. Knob OFF re-confirmed bit-identical with the new instrumentation in place. **The default-on class the original arbitration proposed is hereby exercised for the first time** (leg 4 alone: TF identical over ≥1k real positions) — Q4's own knob doesn't qualify because it reassociates the router and that's what leg 3 caught; a future kernel that leaves the router alone (Q7-gpu's plan) is what the class was written for. Full data (margins, runner-ups, route diff sample) in the record. Original arbitration text follows. **DEFAULT ARBITRATED 2026-09-12 (Fable, record §Q4 `### Arbitration`): the knob stays OFF today and flips ON when ONE chain run meets four legs written there — no second arbitration.** The 3 of 1 200 is neither §G12's class (0 of 1260, and every numerics item on both tracks shipped opt-in: the default-on class has never been exercised) nor §G14's rejection line (greedy changed, cos 0.98964); it is a same-products reassociation proven exhaustively and measured 1.29–1.94× closer to exact than the pristine, so a flip is decisive **only** where the pristine's top-2 margin exceeds the perturbation — a near-tie is two correct kernels disagreeing. Nothing recorded can tell the two apart yet: `Q38_TF_DUMP` writes the last row only. **And the knob reassociates the router** (`q38_dense_matmul(&l->router)` takes the acc4 path; `[OPTIME] router` 2.71 → 2.54), which Q7's spec §3 forbids for the routing decision; routing identity is proved for the 80-token profile run (hit/miss 16364/16699 in all nine runs) and unmeasured for the 1 200-position prompt where the flips are. **Measure (Sonnet, ~1 h rig, kernel untouched):** (1) `diff` the existing `l-A1`/`l-A3` `teacher_forcing` lines → the three positions and whether they sit in the first copy, at a copy boundary, or inside a copy (the long prompt is the short one ×40: ~70 natural positions, ~1 130 induction copies); (2) `tf_margin` + `tf_top2` lines from `q38_tf_emit`, `qp_compare.py` printing margin and runner-up at each flip; (3) `ROUTE_TRACE` on both long-prompt runs, ids diffed over 57 600 rows; (4) one non-repeated ≥ 1 000-token prompt (`MEASURING.md`'s top). **Flips ON iff:** routing identical (0 of 57 600) on both prompts; every flip a near-tie (pristine margin ≤ 0.10, the two argmaxes swapped); natural-text greedy 128 identical and ≤ 3 flips; knob OFF still bit-identical. Routing differs → exclude the router from the knob (−0.17 forgone) and re-run leg 3; still differs, or any margin > 0.10, or the natural prompt fails → opt-in stands, reason filed. A pass amends the default-on class below to "TF identical **or** every change a near-tie with routing identical", the bar Q7-gpu's BF16 `fmt` then meets. Head-only default not offered (no per-kernel attribution of the flips; −1.53 of −10.02). Nothing served moves either way: the gateway is `glm53`. **DONE 2026-09-12, MERGED 2026-09-12** (record §Q4; was `perf/q4-bf16-four-accumulators`). **Every leg of the gate met, and the expected delta landed near the middle of its range.** One engine file, `c/qwen38_core.h` — `glm53.c` does not include it, so glm53 was NOT rebuilt or re-measured (unlike §QP, which touched a shared file). Knob `Q38_BF16_ACC4`, default **0**. **Knob OFF: BIT-IDENTICAL** to pristine — last-token logits byte-equal (993 280 B by `cmp`) and greedy text identical on the 30-token prompt AND the 1 200-position one, `teacher_forcing` 30/30 and **1200/1200**. **Knob ON: greedy text IDENTICAL over 128 tokens after BOTH prompts**, short-prompt `teacher_forcing` clean 30/30, last-token cosine **1.000000000** (relL2 2.18e−6) short and **0.999998812** (relL2 1.54e−3) long, argmax unmoved in both — **but 3 of 1200 long-prompt `teacher_forcing` predictions differ, and that is stated here rather than buried**: it is reassociation accumulating through 1 200 positions of KV cache (§G14 measured 36× more of it, relL2 0.055, and shipped), it is not run-to-run noise (the knob-off pair is byte-identical across two binaries), and the transform oracle measures the candidate kernel as **1.29–1.94× CLOSER to exact long-double arithmetic than the pristine one**, so at those three positions the pristine and a strictly more accurate computation disagree. **Transform oracle first** (`tools/hot-expert/rome_q4acc.c`, §G15's rule): over exact-arithmetic data the two kernels are **bit-identical on 1 332 dots across 36 inner dimensions** (the engine's own plus multiples of 8-not-32, of neither, I<32, I<8) and both equal the exact sum, **0 mismatches**, plus 2 048 more at S=32 and a negative control that fires. **Isolated** (`rome_cpubench.c`, 8 threads, pools > L3 and > Infinity Cache; the gated residual's 10240×320 and 320×10240 added for this item): **1.06–1.17× at S=1** on every dense shape including the short-I pair, **3.07–3.24× at S=32**. **In-engine:** `[OPTIME] dense-matmul` **89.06 → 80.56 (−8.50)**, `lm-head` **15.45 → 13.93 (−1.53)**, bucket **−10.02**, `step` **175.60 → 165.80 (−9.79)**, every untouched sub-bucket flat (dn-conv +0.02, gr-mix −0.00, cpu-experts +0.09, vk-issue +0.02) and `vk-take` **+0.30** in the expected direction; **all nine profile runs share the identical expert hit rate (49.5 %, hit=16364 miss=16699)**, so the timing pair is controlled in §G14's sense. **Attenuation is ~1.00×, not §Q-PROFILE's 0.95× and nothing like §G14's 0.55–0.65×.** **Serving A/B** (`rome_bench.sh`, interleaved, two repeats): rotating **4.41 / 4.38 → 4.54 / 4.55** (+3.4 %, both candidate runs above both pristine), warm-identical **5.63 / 5.62 → 5.96 / 5.96** = **−9.99 ms/token**, agreeing with the counter to 0.3 ms. **Free extra:** the 30-token prefill forward **4 318 → 2 949 ms (1.46×)**, which is the old Q5 prefill claim measured rather than extrapolated. **ONE THING LEFT FOR FABLE, NOT DECIDED HERE: whether the default should flip.** The knob ships off, so the −10 ms/token is not in the default binary; the case for turning it on is stronger than anything this record has shipped opt-in, and the case against is the 3 of 1 200 — a decision about the engine's default output, not one the session that wrote the kernel should take. Original row follows. **RANK DECIDED 2026-09-12: QP(b) died, so this row's own conditional fires — "if int8 dies, Q4 is the only CPU-side gain left on the 106 ms and goes first". It is now the first item on the track** (record §QP (b)). It also inherits the whole 105.8 ms/token rather than sharing it: Q7-cpu is dead, so nothing else is going to take these bytes on the CPU, and the "not additive with Q7" caveat now means only "not additive with Q7-**gpu**, which moves the same bytes to dev0". Original row follows. **Four accumulators in the BF16 dense GEMV at S=1** (was Q5, which said "S>1 only, TTFT not decode" — **measurement says otherwise**). Both kernels need it: `q38_matmul_bf16` and the inlined loop in `q38_dense_matmul_multi`, 30.49% and 24.91% of user cycles. Breaks the single FMA dependency chain; **changes the summation order**, so it ships behind a knob with the logit diff recorded. **Its rank is decided by QP(b):** if int8 dense survives, Q7-cpu replaces this kernel on the tensors that matter and Q4 becomes what it was originally — a prefill item (1.8× at S=32) — and moves behind Q7; if int8 dies, Q4 is the only CPU-side gain left on the 106 ms and goes first | `dense-matmul` **90.0** + lm-head **15.8** = **105.8 ms/token (55%)** | `rome_cpubench.c`, 8 threads, pools > L3, this engine's exact shapes: **79.6→90.8, 81.8→90.2, 80.7→87.9, 84.7→92.7 GB/s — 1.10–1.14× at S=1**. In-engine attenuation on *this* path is ~0.95×, **not** §G14's 0.55–0.65× | **−8 to −12 ms/token** on today's kernel; **not additive with Q7** — both act on the same bytes | 1 day | Opus | knob **off** bit-identical to pristine; knob **on**: greedy text identical over 128 tokens and the last-token logit cosine in the commit body; `[OPTIME] dense-matmul` and `lm-head`; rotating median |
...
| **Q9** | **CHUNK PROBE RUN 2026-09-12 — the probe as specced does NOT answer this item; two findings, neither an MTP verdict (harness `tools/hot-expert/q9_probe.sh`; the two accounting increments are NOT in the tree — they lived on the retired branch `perf/q9-chunk-probe`, `cb25432`, and must be re-applied to re-run this).** (1) The `[OPTIME] placement` counter is never incremented in the chunked prefill path (`q38_moe_prefill`, `Q38_PREFILL_BATCH=1`) — only the decode/row-at-a-time path counts — so the first probe read `gpu=0 cpu=0` for the batched leg; the two accounting increments are added on the branch. (2) With the counter fixed, the chunked path places **390 / 41 026 distinct experts on the GPU (0.95 %)** against **111 971 / 119 040 (94 %)** row-at-a-time: the chunked path's GPU placement is gated `load_count <= 64`, and a 240-token prompt produces ~114 distinct experts per chunk, so nearly every group skips the tier. **`Q38_PREFILL_BATCH=0 vs =1` is therefore a confounded A/B — it switches two different MoE implementations (placement strategy), not just the chunk size — so the dedup question (SPEC-PROBE's flat-35 252 test) is still unanswered.** **DONE 2026-09-15 (Opus; commits `3b1f54d`, `df3ccf0`) — and neither of this row's own proposed fixes was the right one.** The gate was not too small, it tested the WRONG QUANTITY: `if(g_q38vk_ready && load_count<=64)` measures the TOTAL distinct experts in the group, while 64 is the PER-DEVICE limit of `eg_prepare_submit` and what reaches it is `bufN[dev]`, after the split across three devices. ~114 distinct experts split ~38/38/38 is legal on every device and was refused anyway. The limit now sits per device beside the scratch-exhaustion check that already uses the same "leave unplaced, CPU handles it" path; **nothing in `backend_vulkan.c` changes and no buffer grows** (every `buf*[3][64]` is indexed by `bufN[dev]`, which the cap keeps under 64), so both options this row previously recommended — sub-batching, and raising 64 throughout the backend — are unnecessary. The earlier "enlarge the buffers" instruction would have been a silent no-op: `eg_prepare_submit` REFUSES `count>64` rather than overflowing. Also fixed: `[OPTIME] placement` never incremented on the chunked path, which is why the first probe read `gpu=0 cpu=0`. **Measured** (Qwen3.8-Flash-Next-FP8, 1225-byte prompt, `Q38_PREFILL_BATCH=1`, tier 14673/21858, fully resident, arms run in BOTH orders because the first pair was order-confounded): TTFT **45.44/41.45 s → 27.02/27.29 s**, i.e. **−34%** on base's better number and −40% on its worse; the candidate is stable to 0.3% across positions, base is not. Expert cache hit rate **70.0% → 31.6%** with misses IDENTICAL at 16620 in all four runs — the same experts are needed, ~31k accesses simply stop being served from the CPU cache. **Oracle:** `teacher_forcing` identical, logits max_abs 3.672e-05, cos 1.000000000, argmax unmoved — reassociation, not divergence, and notably NOT the shape GLM shows between its expert paths (cos 0.99/0.98, argmax moves; §G15). Knob `Q38_CHUNK_TIER_TOTAL_GATE=1` restores the old gate. **MTP QUESTION ANSWERED 2026-09-15, and it goes the OPPOSITE way to GLM's.** With the gate fixed the chunked leg actually uses the tier (87.77% on it, against 0.95% before), so `Q38_PREFILL_BATCH=0 vs =1` is finally a comparison of chunk size rather than of two placement strategies, and the probe `tools/hot-expert/q9_probe.sh` was re-run on it: **prefill CPU experts K=1 → 7069, chunked → 5016, delta −2053 (−29.04%)**. (Totals: b0 gpu=111971 cpu=7069 at 94.06% on the tier; b1 gpu=36010 cpu=5016 at 87.77%.) **VERDICT: DEDUP — adjacent tokens DO share cold experts on Qwen, the verify-of-K amortises the miss, and by this row's own decision rule that makes MTP worth a Fable spec.** This is the reversal §SPEC-PROBE's ~1.2× cap did not anticipate: GLM measured FLAT (35 252 CPU experts at every K from 1 to 16, i.e. no sharing, MTP refused), and the same probe on Qwen finds a 29% reduction. The two engines differ on the one property the whole MTP family depends on, so GLM's null hypothesis does not transfer. **Caveat, stated rather than buried:** the two legs still differ slightly in tier share (94.06% vs 87.77%), so a residual placement component in the 29% cannot be excluded; it is nothing like the 94-vs-0.95 confound it replaces, but a spec should not treat 29% as exact. **Q9 step 1 is complete. What remains of this row is the Fable spec and the build, which are a separate decision, not a measurement.** that is also a prefill/TTFT item in its own right (the chunked prefill leaves the tier nearly unused). Previous row text follows. **MTP speculative decoding** (was Q4). **⚠ RE-GATE BEFORE STARTING. The "GATED BEHIND Q8" clause is now moot: Q8 is dead (QP(a), record §QP), so SPEC-PROBE's reversal condition — "after Q8 the miss share is the never-routed tail" — never fires, and §SPEC-PROBE's ~1.2× cap stands as this row's null hypothesis rather than something Q8 was going to lift.** That makes the chunk probe (step 1) the whole decision, and it makes this row *cheaper to refuse* than it was: it is half a day to find out. Original row follows. **⚠ RE-GATE BEFORE STARTING and GATED BEHIND Q8.** Load the MTP module, draft up to 4 tokens per step reusing the QSA top-k indices, verify in one forward, accept the matching prefix (tech report Table 4: 4.06 mean accepted) | — | §SPEC-PROBE capped this family at ~1.2× optimistic **for GLM**, because the experts that miss the tier are cold ones adjacent tokens never share. Qwen's miss share is **19.2%**, close enough to GLM's 21% that the same cap is the null hypothesis — **but after Q8 the miss share is the never-routed tail and SPEC-PROBE's own reversal condition fires** | largest multiplier on the board, **after Q8 and not before** | chunk probe half a day; spec 1 session + 5–8 days | **Fable** (spec) then Opus + Sonnet | **step 1 is the chunk probe on qwen38** (`Q38_PREFILL_BATCH` × the `[OPTIME] placement` counters): report the CPU-expert count at block size 1→16. Then accepted text == greedy text for 1k tokens; accepted length ≥ 3.5; rotating tok/s |
| **Q10** | **DONE 2026-09-12, MERGED 2026-09-12** (record §Q10 step 0 and §Q10; was `perf/q10-deltanet-recur-traffic`). **Every gate leg met, no arbitration needed — and the item's OWN premise was wrong in three separate places, all three caught by step 0 before a line of engine code was written.** Measured: `dn-recur` **9.169 → 1.974 (−7.195, 4.6×)**, `deltanet` **64.157 → 57.027 (−7.130)**, the token **171.914 → 163.416 (−8.498)**, last-token logits byte-identical (993 280 B) at 32 with timers off and on, at 128 and on the long prompt, `teacher_forcing` identical over 30 and **1 200** positions, greedy text identical, and the harness's full-state oracle **0 of 1 811 939 328** elements over 36 layers × 64 consecutive tokens. Rotating **4.37 → 4.48 (+2.5%, both candidate runs above both pristine runs on all three rows)**, warm-identical **5.65 → 5.87 (+3.9% = −6.6 ms/token)**. **(1) The sizing was wrong a third time, and this time in the item's FAVOUR.** Step 0 tested the two mechanisms separately: folding **(a)**, which removes a full third of the state traffic (6 → 4 units) and is what the roadmap's **−2.5 was sized on, is worth −0.09 / +0.34 / −0.14 ms/token** in the L2 / L3 / DRAM regimes — **zero**; folding **(b)**, the `d`-outer/`value`-inner loop order, is worth **−4.2 to −4.3 in every regime**, the same number whether the state is in L2, L3 or DRAM, which is what a latency bound looks like and a bandwidth bound does not. A control with (b)'s order and the decay pass **kept** (5 traffic units) measures the same, so **nothing in the result is traffic**. (a) is **rejected and deleted** as a separable mechanism; it survives in the kernel only because (b) needs the on-the-fly `fl(s·α)` anyway. **(2) The corrected latency mechanism was ALSO wrong:** the row says the reductions are "unvectorised across `value`", and the assembly shows gcc had already outer-loop vectorised both. What (b) fixes is ILP and access order — one ymm accumulator down a 128-long dependent `vaddps` chain with a 512 B stride becomes 16 independent accumulators over unit-stride loads. `perf stat` agrees from the counters: the base retires **4.1–4.9× more instructions** at **IPC 1.20** past L3 (not memory-stalled — executing) while the fold runs at **IPC 0.65 with 3.7× MORE cache misses and is still 2.2× faster**. **(3) Both numerics claims in the row are FALSE and the oracle caught both.** "The addend of an FMA is never contracted" — spelled as `s*alpha + kh[d]*delta[v]`, gcc fuses the OTHER multiply and **480 790 578 of 1 811 939 328** state elements differ; and because gcc does *not* contract the base's two strided reductions but *does* contract a d-outer `acc[v] += x*y`, (b) in its obvious spelling differs in **208 336 749** (localised statement by statement by two diagnostics: (b) on `current` alone changes **0 state elements** and only the output). The shipped form states contraction explicitly in both directions — `fmaf()` where the base fuses, `#pragma GCC optimize ("fp-contract=off")` around the function where it does not — and both obvious spellings are deleted. A **once-per-process self-check** runs both forms on a fixed state and falls back to the base if a compiler ever disagrees; the two kernels are `noinline` so the check exercises the code the engine runs, and that costs **nothing** (`dn-recur` 9.169 → 9.161, step 171.914 → 171.616 with the knob off). `Q38_DN_RECUR_FOLD=0` is the A/B. **The gate was re-derived at step 0 and NOT set at the track's usual "half"**: half of 9.19 is 4.60, which sits *above* the mechanism's own conservative prediction of 4.94 and would have been the §Q1/§Q2/§Q3 mistake a fourth time; the floor was set at **≤ 4.94** and met at **1.974**. **One finding for the rest of the track, and it settles a §Q1-vs-§Q2 disagreement:** in-engine attenuation runs in **opposite directions for the two forms of the same loop at one call site** — the base is 1.34× WORSE in the engine than isolated, the fold is 0.76× BETTER, and the saving is 1.69× larger in the engine than the harness predicted. It tracks **how well the access pattern survives a cold cache and TLB**, not serial-vs-parallel and not libgomp. So **an isolated harness understates a loop-order fix and can overstate a traffic fix**, and a latency-bound loop should be sized from the in-engine bucket and the isolated *ratio*, not the isolated absolute delta. **My own greedy oracle was wrong first and it is recorded, not quietly fixed** (§QP's precedent): the comparison range ended at the `TTFT:` line, so a stopwatch could move it — it called the knob-ON continuation divergent while every logit dump was byte-identical. Also recorded: **`tests/test_qwen38_native_weights` fails identically on the pristine** (`:587`), pre-existing and not this item's; the other six qwen38 tests pass on both sides. Original row follows. **The DeltaNet recurrence's state traffic** (new, logged at Q1's arbitration 2026-09-12 from Q1's step-0 sub-timers, record §Q1 "What this changes" and `### Arbitration`; not sized by a measurement yet). `dn-recur` is 9.19 ms/token and already 48-way parallel; per head and token it makes four traversals of a 128×128 f32 state (64 KB): decay `state *= α` (read+write), `k·state` (read), the rank-1 update (read+write), `q·state` (read). Two foldings, both order-preserving per element: **(a)** drop the decay pass — compute `fl(s·α)` on the fly inside `k·state` and again inside the rank-1 update as its addend (`state = fl(s·α) + k·δ`; the addend of an FMA is never contracted, so the value is the same float both times): 6 → 4 traffic units; **(b)** fold `q·state` into the rank-1 pass with `d` outer and `value` inner: 4 → 3, every `current[value]` still accumulates in ascending `d`. **Sizing, corrected at arbitration:** §Q1's "680 MB/token at 74 GB/s, the memory ceiling" assumes all four traversals hit DRAM, but a head's state is 64 KB and this CPU has **512 KB of L2 per core**, so traversals two to four are L2-served and DRAM sees ~226 MB/token (~25 GB/s). The likelier cost is the two **`d`-inner strided reductions** (`previous`, `current`: stride 512 B, 128 dependent adds per output, unvectorised across `value`), which (b)'s loop order also removes | dn-recur **9.19 ms/token** (§Q1), 4.8% of the token | Q1's sub-timers; the traversal arithmetic above; `lscpu` | **−2.5 ms/token** on the bandwidth reading; **more on the latency reading** — step 0 says which, and the gate is derived from it, not from this cell | step 0 half a day; the change half a day | Sonnet (Opus if the vector form of (b) needs checking against contraction) | **step 0 first**: the recurrence added to `rome_dnbench.c` with its oracle extended to the state, timed L2-hot against cold, and `perf stat` cycles against bytes — the gate is then written as a fraction of the 9.19 (per the "(est.)" rule) before the change is built; **bit-identical** (the microbenchmark's `memcmp` on the state over 36 layers × 64 tokens, then greedy text + last-token logits — contraction must match in both forms, and the oracle decides, not the argument); `[OPTIME] dn-recur` and `deltanet` reported, untouched sub-buckets flat; serving A/B with its own pristine rows — **step 0 done and the gate re-derived from it before the candidate (record §Q10 step 0): `dn-recur` ≤ 4.94, met at 1.974; bit-identical met at every leg; `deltanet` −7.130 against `dn-recur`'s −7.195; untouched flat (max +0.049); token −8.498; rotating 4.37 → 4.48 — every leg met, merged, no arbitration** |
| **Q11** | **REJECTED ON MEASUREMENT 2026-09-12, DO NOT RE-OPEN** (record §Q11 and §Q11 step 0; branch `perf/q11-gr-apply-writeback`, engine file reverted to the pristine, nothing shipped). **Five arms, one campaign, every one of them bit-identical (8 oracle pairs, byte-identical logits at 32 and 128 tokens, plus a memcmp on `hyper` itself in the harness) — and none of them worth shipping.** `gr-apply` + `gr-rms` are **1.24 ms/token** today; the shape THIS ROW DESCRIBES (a pragma on each loop, two regions) lands them at **1.71 (+0.47)**, the naive write-back pragma alone at **1.96 (+0.72)**, and the best shape that exists — both `omp for`s fused into ONE region, both split over `b` so thread `b` reads back the 10 KB it just wrote — at **1.14, i.e. −0.10**, on a token whose run-to-run spread is 0.6. **The gate leg `gr-apply ≤ 0.10` is NOT MET (0.56 best) and is UNREACHABLE by any pragma**: `rome_grapplybench.c` measures an empty `omp parallel` at **4.51 µs** and a second `omp for` inside a region at **4.17 µs** more, so one fork/join per site is 0.43 ms/token — four times the threshold before any work is done — and the cheapest two-loop shape costs 9.3 µs/site of synchronisation against 12.8 µs/site of work. **This row's mechanism is refuted, not merely unprofitable:** "with this loop parallel the invalidation is paid eight ways and the rms half becomes free" is false — arm 3 is exactly that and is +0.47. What IS true and is now in the record: the 0.18 → 0.53 transfer §Q2 found is **a decomposition mismatch between whoever writes `hyper` and whoever reads it next, and it runs in both directions** (serial writer + 4-way reader 0.54; 8-way writer + serial reader 0.90; 4-way writer + 4-way reader, aligned, 0.56). **The general bound this leaves Track Q: on this box a loop is worth a pragma only above ~10 µs of work per call site** — price any future pragma row against 9.3 µs/site of synchronisation before scheduling it. No serving A/B was run and none was possible: 0.10 ms/token is 0.06 % of the token against a rotating row that drifts ±3 % on unchanged code. **§Q2's "the 0.33 left on the table" is withdrawn.** Original row follows. **The gated residual's write-back, and the 0.35 ms/token §Q2 could not keep** (new, logged at Q2 2026-09-12, record §Q2 "the rms half, rejected" — and unlike most new rows its measurement is already taken). `q38_gr_apply` is a serial scalar loop at 96 sites per token: `for b<C for d<H: hyper[s*W+b*H+d] += inject[s*C+b]*block[s*H+d]`, 10 240 fused multiply-adds per site, **no reduction at all**, so `collapse(2)` over (b,d) or over (s,b) is bit-identical by construction — the easiest pragma left on this engine. Its own bucket is small (0.15–0.18 ms/token) but it is the **precondition for re-applying §Q2's rejected rms pragma**: §Q2 measured that parallelising `q38_gr_read`'s four `q38_rms0` calls wins 0.33 in `gr-read` and loses 0.35 in `gr-apply`, because four threads pull all 40 KB of `hyper` into four private L2s and this loop then writes all of it from one thread, paying cross-CCX invalidation on this 4-CCX EPYC. With this loop parallel the invalidation is paid eight ways and the rms half becomes free. **Do both halves in one item and gate them together**, or the second half will look like a regression again | gr-apply **0.15–0.18 ms/token** (96 sites) **plus** the 0.33 §Q2 measured and left on the table | §Q2's three-arm in-engine table (pristine / rms+mix / mix only: `gr-apply` 0.18 / 0.53 / 0.17, `gr-rms` 1.07 / 0.82 / 1.07, step 179.29 / 174.65 / 174.76), medians of three repeats, reproducible to ±0.01 in two campaigns | **−0.4 to −0.5 ms/token** (0.15 from this loop + 0.33 from the rms half, less one fork/join per site) — small, and it is the cheapest remaining serial scalar loop on the engine | half a day | Sonnet | **bit-identical** (no reduction anywhere in either loop, so this is by construction; greedy text + last-token logits anyway); `[OPTIME] gr-apply` ≤ 0.10 **and** `gr-rms` ≤ 0.85 **in the same binary**; **report `gr-apply` and `gr-read` together and the token beside them** — §Q2's lesson is that either loop alone moves time between these two buckets; serving A/B with its own pristine rows — **VERDICT 2026-09-12: bit-identical MET (all five arms, 8 oracle pairs); `gr-apply ≤ 0.10` NOT MET (0.56 best of five) and shown UNREACHABLE by any pragma; `gr-rms ≤ 0.85` met (0.58) but only in arms that fail the other leg, so the "same binary" requirement is NOT MET; buckets-and-token reported together as required; serving A/B deliberately not run because 0.10 ms/token cannot be resolved by a row that drifts ±3 %. REJECTED, engine reverted, nothing shipped** |
| **Q12** | **PARKED 2026-09-12 (owner decision): declined on expected value, not killed by measurement — not run.** QP already measured int8 dense dead (cos 0.9802/0.9821, §QP); the only prize left is the gated-residual pair (~−10 ms/token) behind QP's unsoftened kill line on the subset Fable already calls least likely to survive, and the item is building (a filter knob plus multiple probe sweeps) rather than measuring — not worth opening two days before a rig reset. **Not dead: re-open only if a concrete reason to know the int8 boundary appears.** **REVISITED 2026-09-15, THE CONDITION IS MET, AND THE ANSWER IS UNCHANGED: STAYS DECLINED.** Q5, Q9 and Q13 all closed the same day, so the hold expired as specified. Re-reading the park against today's evidence: the timing premise ("not worth opening two days before a rig reset") HAS expired, exactly as predicted — but it was the *additional* reason, not the load-bearing one. The load-bearing reasons are untouched: int8 dense is dead on measurement (cos 0.9802/0.9821, §QP); the only prize left is the gated-residual pair at ~−10 ms/token, sitting behind QP's unsoftened kill line, on the subset Fable itself called least likely to survive (194 tensors at 97 sites, against a line one tensor has already failed); and the work is *building* — a filter knob plus multiple probe sweeps — rather than measuring, which is the worst shape to spend a session on for those odds. Nothing measured since the park moved any of that. **Declined on standing evidence, not killed**, and the re-open condition is unchanged and deliberately cheap to trip: a concrete reason to know the int8 boundary. Previous text follows. **SEQUENCING CONFIRMED BY THE OWNER 2026-09-15: hold Q12 until the other Q items are finished (Q5, Q9, Q13), then revisit — do not re-litigate the park before then.** Note for whoever picks it up: the deciding factor recorded above was *timing* ("not worth opening two days before a rig reset"), and that premise expires on its own; the odds (the subset Fable calls least likely to survive, against a kill line one tensor already failed) and the build cost do not. Q9 is next. Previous row text follows. **CORRECTED AT Q7's ARBITRATION 2026-09-12 (Fable, record §Q7 `### Arbitration`): what Q7 step 0 removed is this item's PLACEMENT consequence, not its prize.** The pair stays on the CPU whatever an int8 subset shows (the GPU pair is slower than the CPU pair; step 4 refused), so nothing here changes what moves to dev0 — but the pair is **18.45 ms/token** of CPU BF16 and an int8 pair is ~7.9, so a surviving subset is worth **~−10 ms/token** on the token, more than step 4 ever promised. It is also the subset least likely to survive (194 tensors at 97 sites, against a kill line one tensor already failed, §QP (c)), and it is a plain CPU-precision probe under §QP's unsoftened gate. Not closed; sequenced after Q7 steps 2–3 and Q13. Previous row text follows. **RE-SEQUENCED 2026-09-12 (Opus, from Q7 step 0's measurement; record §Q7): its one remaining lever is GONE, so this item is now worth nothing on the token and should be closed unless someone wants the boundary for its own sake.** The Q7 spec's sequencing argument ended "even a survivor beats the GPU BF16 arm on the token only for the gated-residual pair, which is Q7's last step". **Q7 step 0 measured that pair on the GPU at 21.2–38.1 ms/token against its 18.45 on the CPU** — the GPU arm is slower than the CPU arm there, so the pair stays on the CPU whatever Q12 finds, and Q7 step 4 was refused. A surviving int8 subset could still make the pair's CPU arm faster (~7.9 vs 18.45), which is the ONLY thing left for this item — but that is now a plain CPU-precision question with no placement consequence, and it has to clear QP's unsoftened kill line to claim it. Previous row text follows. | **SEQUENCED 2026-09-12 (Fable, Q7 spec §"Q12 sequencing"): does NOT gate Q7-gpu.** One int8 tensor already fails QP's greedy leg (the head, §QP (c)); the depth amplification is a random walk (676 tensors = 26× one tensor ≈ √676), so a subset worth ≥ 1 GB (≥ 36 tensors, ~6×) sits inside the failed class; and even a survivor beats the GPU BF16 arm on the token only for the gated-residual pair (CPU int8 ~7.9 vs GPU ~9–12 + 1.2 ms), which is Q7's last step. Run it in the gap before Q7 step 4 if rig time allows; it can move the pair's placement and nothing else. Original row follows. **How much of the dense set DOES take int8? — the subset question QP(b) did not ask** (new, logged at QP 2026-09-12, record §QP (b) "why depth, not per-tensor precision"). QP(b) quantised all **676** BF16 layer tensors at once and the model failed; the same measurement also showed **why**, and the why is the item: the int8 weight perturbation is **relL2 3.9e−3**, one int8 tensor (the head, QP(c)) moves the logits by **7.7e−3** — about 2×, no depth — and the identical per-tensor perturbation applied across **48 layers** moves them by **0.198**, a **~50× amplification that is entirely depth**. So "int8 the dense set" and "int8 a dense tensor" are different questions and QP answered only the first. Find the largest subset of the 676 that clears QP's gate, biggest-bucket-first: `dn-proj` is ~52.8 ms/token of the 90.0 in `dense-matmul` and the gated-residual pair is 17.8, so even a partial subset is worth real time on the CPU — and if a subset *does* clear it, Q7-cpu comes back from the dead for exactly those tensors | `dense-matmul` **90.0** (of which `dn-proj` 52.8, the GR pair 17.8), lm-head **15.8** | §QP's depth arithmetic above; the whole apparatus already exists — `Q38_I8_DENSE`, `Q38_I8_ROUTER`, `Q38_TF`, `qp_probe.sh`, `qp_compare.py`, and a transform oracle that is bit-identical to `quantize_rows` | unknown by construction — **this is a probe, and "no subset survives" is a complete answer** | **step 0 is a tensor-group filter knob (half a day, Sonnet); the probe itself is one qp_probe.sh sweep per subset (~25 min of rig time each)** | Sonnet for the knob, Opus for the verdict | the filter knob **off bit-identical**; then QP's own gate, unchanged and unsoftened, per subset: short-prompt `teacher_forcing` unchanged, long-prompt (≥1000) cos ≥ 0.99, greedy identity over 128 tokens; **report every subset tried including the ones that failed** — the boundary is the result, not the first subset that passes |
| **Q13** | **MEASURED END TO END 2026-09-15 AND HANDED BACK TO THE OWNER AS A STANDING TRADE — the gain is real, the price is real, and the box is back on `auto`.** The owner set dev0 (`0000:83:00.0`) to `high` by hand (the sysfs node is root-owned; no passwordless sudo, so this row cannot be executed unattended) and reverted it after two runs. Decode track, same binary and harness as that morning's `auto` pair, so this is a same-day A/B: **`auto` 3.03 / 2.98 → `high` 3.18 / 3.32 tok/s rotating** (warm 3.44/3.45 → 3.73/3.85; cold 1.86/1.90 → 1.99/1.99). Conservative reading **+5.0%** (worst `high` against best `auto`), optimistic +8.1% on the means; the `high` pair is noisier (4.4% spread against `auto`'s 1.7%) so the conservative number is the one to quote. That brackets §Q13 step 0's predicted −8.281 ms/token (−7.1%, i.e. +7.6% in tok/s). **Price, measured not quoted: dev0 idle 8.0 W → 75.0 W, so ~+67 W standing** whenever the rig is powered and idle — on a 24/7 gateway that is ~1.6 kWh/day for ~6% on the token. **Disposition: not a code change, nothing to merge, and not a decision an agent should take** — it is a permanent power commitment on the owner's electricity bill. Left at `auto`. To re-enable: `echo high | sudo tee /sys/bus/pci/devices/0000:83:00.0/power_dpm_force_performance_level` (use the PCI path, not `/sys/class/drm/cardN` — the card numbers are scrambled and move across reboots, see §Q13's own note). It does not survive a reboot; a udev rule keyed to the PCI address would make it persistent if the owner ever wants that. Previous row text follows. **STEP 0 DONE, BOTH DPM VARIANTS MEASURED — IT IS CLOCKS, AND IT IS THE *CORE* CLOCK, NOT THE MEMORY CLOCK (2026-09-12, Opus; record §Q13 "The narrow variant").** The record's own "cheaper variant" prediction is **REFUTED and the escape route is closed**: `manual` + `pp_dpm_mclk=3` (mclk pinned 1249, sclk free) pays **93 % of `high`'s idle-power cost (+64 W of +69 W; idle 72–73 W against `auto`'s 9 W and `high`'s 78 W)** and returns **at most 12 % of its gain on any bucket and nothing distinguishable on the token** — mask 1 token −0.387 and mask 7 −0.391 with the repeat ranges **overlapping**, against `high`'s −4.302 / −8.281 with no overlap; only `dn-proj` / `vk-dense` separate, at ~0.33 ms/token, and `lm-head` moves the wrong way. Oracle bit-identical 6/6 for this variant too. **So a memory-bound GEMV's attenuation is carried by the shader clock's ramp in the gap, not the memory interface's** — the item's own inference from "GEMV is memory-bound" did not survive its measurement. **Consequences: neither DPM variant is worth shipping as a rig default on these numbers** (−8.281 ms/token on an engine that is NOT served, for +69 W permanently on the card that carries the console), **and the keepalive dispatch is now the only live lever and a better-aimed one** — it holds the *core* busy, which is the clock identified, and costs no idle watts when the engine is idle; ceiling −8.281 less its own cost in the gap. Not built (separate Opus pass). One discriminator left if the mechanism is wanted nailed rather than inferred: `manual` + `pp_dpm_sclk=2` with mclk free, one sudo pair and 15 min. **dev0 needs reverting to `auto` — still owed.** Previous rows follow. **STEP 0 DONE — IT IS CLOCKS, ON A PAIRED SAME-BINARY A/B WITH A BIT-IDENTICAL ORACLE (2026-09-12, Opus; record §Q13 step 0 "The paired A/B").** dev0 `auto` vs `high`, tip binary `5a9a2495`, three fresh processes per mask, **no overlap between the two levels' repeats on any moved bucket**: **mask 7 (the arm that ships) the token 116.531 → 108.250 = −8.281 (−7.1 %)**, `dn-proj` 22.304 → 18.843, `qsa-proj` 8.176 → 7.065, `lm-head` 3.363 → 2.638, the arm 33.843 → **28.546**, `vk-dense` 33.806 → 28.518, `vk-take` 5.990 → 4.806; **mask 1 the token 125.341 → 121.039 = −4.302**, `dn-proj` **20.030 → 17.295**. The row's own test — "if `dn-proj` moves toward 19.6 it is clocks" — is met at both masks, and the expected −5 to −10 ms/token is met at mask 7. **Oracle: `Q38_TF_DUMP` and `DUMP` sha256-identical across the levels, 6 of 6, both masks — a DPM level changes no arithmetic, measured.** **Idle: dev0 9 W at `auto` (mclk 96 MHz) vs 78 W at `high` (mclk 1249) — +69 W permanently, ~600 kWh/yr, on the card that also carries the console, to speed up an engine that is NOT served.** That is a policy decision, not a measurement, and the gate's last leg (serving A/B with its own pristine rows) is worth taking only after it. **Two corrections to §Q7 fall out of the paired baseline: Q10 alone removed ~6.9 (mask 1) / ~4.9 (mask 7) ms/token of `dn-proj` on the GPU path — its own gate saw `dn-proj` flat because that was the knob-OFF CPU kernel — so §Q7's duty-cycle replay UNDER-prices the A→B gap by ~8×, and its "back-to-back control" (18.61) is not a floor: the engine reaches 17.295 at `high` because the control was itself measured at `auto`. The in-engine attenuation after Q10 is 1.158×, not 1.37×.** Cheaper variant now indicated rather than guessed: idle power is almost entirely mclk, so `manual` + `pp_dpm_mclk=3` should buy most of the −8.3 at a fraction of the watts — one sudo pair and one 15-min run. **The keepalive dispatch is NOT built (separate Opus pass); its ceiling is now measured at 8.3 ms/token** less its own cost in a gap §Q3 showed is unevenly distributed. **For Q14: supported in kind, bounded in size — `gr-read` moves −0.926 at mask 7 (no overlap) and is FLAT at mask 1, i.e. it moves only where `vk-dense` is large, which is §Q14's own fence-wait correlation; 0.93 of its +6.35 is dev0's power state and ~5.4 is not.** dev0 left at `auto`. Previous rows follow. **STEP 0 HALF TAKEN — IT IS CLOCKS, PROVISIONALLY; THE `high` LEG IS MEASURED AND THE PAIRED `auto` LEG IS NOT (2026-09-12, Opus; record §Q13 step 0 "The `high` leg").** At `power_dpm_force_performance_level=high` on dev0, tip binary `5a9a2495`, three fresh-process repeats a mask: **mask 1 `dn-proj` 17.295** (was 26.979 at `auto` on the pre-Q10 binary) and the token 121.039; **mask 7 `dn-proj` 18.843, `qsa-proj` 7.065, `lm-head` 2.638 — the arm 28.546 against 38.880 (−10.33)** and the token 108.250 (was 127.740). **Two of those buckets are untouched by Q10** (`qsa-proj` −14 %, `lm-head` −24 %), and **the in-engine numbers at `high` now sit BELOW the isolated microbenchmark** (17.295 vs `bench_q7_dutycycle`'s 18.61 back-to-back and step 0's 19.6; 2.638 vs 2.74–2.85) — the replay was itself run on a de-clocked device, so §Q7's 1.37× was under-measured, not over. **Idle power is the price: dev0 78 W at `high` against 6–7 W at `auto`, mclk pinned 1249 vs 96 MHz, +71 W ≈ 620 kWh/year on a 24/7 box — the owner's call, and the reason `manual` + `pp_dpm_mclk=3` (memory clock only) is worth trying if this is pursued.** **NOT done, and no gate leg is claimed met:** the `auto` leg on this binary needs the card reverted (`echo auto \| sudo tee /sys/bus/pci/devices/0000:83:00.0/power_dpm_force_performance_level`, **still owed as of this row**), then `WT=/home/ronald/src/colibri-q10 Q13_ORDER=auto Q13_FINAL_AUTO=0 ~/bench/q13_probe.sh` (~15 min) pairs the table and closes the bit-identity oracle; the serving A/B is after that. The keepalive dispatch is NOT built and is the separate follow-up. **For Q14:** `gr-read` moved 27.96–28.42 → 26.07 when only a GPU's DPM level changed, so the link is real in kind, but it stays ~4 ms above its knob-off 21.86 — most of Q14's +6.35 is not dev0's power state and Q14 keeps its own hypothesis. Device-map prep follows. **STEP 0 PREPARED, MEASUREMENT BLOCKED ON THE OWNER'S SUDO (2026-09-12, Opus; record §Q13 step 0).** The thing the item could not be run without is settled: **dev0 is `0000:83:00.0` = `/sys/class/drm/card1`** (dev2 `0000:48:00.0`, dev3 `0000:86:00.0`), proved three ways — the engine's own selection loops replayed against `VK_EXT_pci_bus_info`, the live gateway's VRAM ledger (83 holds 22.85 GB against 24.19 GB on each of the two expert-only tiers), and `coli_vk_matmul_multi()` being dev0-only. The card numbers are scrambled (card0=86, card1=83, card2=48) and the engine's `/dev/dri` fd order (48, 86, 83) is RADV's scan order, **not** the enumeration order — a session that reads it as the device order forces the wrong card. **Idle at `auto`, model resident: dev0 6–7 W, sclk 0 MHz, mclk 96 of 1249 MHz (7.7 %)** — the suspected mechanism, quantified but not yet decided; dev0 is also the console/display card (`fb0`, the one connected HDMI). Harness `tools/hot-expert/q13_probe.sh` (= `~/bench/q13_probe.sh`) is ready: lock, gateway, residency assert, 3+3 fresh-process repeats at `Q38_DENSE_GPU=1` and `=7` per level, 2 Hz clock/power sampling, sha256 bit-identity between levels, and it **polls** the sysfs level instead of writing it. What is needed: `echo high \| sudo tee /sys/bus/pci/devices/0000:83:00.0/power_dpm_force_performance_level`, then `auto` to revert. Original row follows. **dev0's idle clocks between dense submits** (new, logged at Q7's arbitration 2026-09-12 from step 1's duty-cycle measurement, record §Q7 "Why it undershoots" and `### Arbitration`). `bench_q7_dutycycle` runs the identical two DeltaNet submits at **18.61 ms/token back-to-back and 27.46–34.67 with the engine's own 0.26 / 1.50 ms gaps**, nothing else changed; the cost sits in the long gap (B → the next layer's A: 8–16 ms/token), not the short one (0.4–0.9), and is non-monotonic in gap length (22–24 at 3.0 ms) — a power-state signature, not a throughput one. Every submit the engine makes pays some of it; the expert groups' `vk-take` (8.2 after Q7 step 1) may carry a ramp too. **Step 0, zero code:** `power_dpm_force_performance_level=high` on dev0 (sudo — the owner), the step-1 `[OPTIME]` triple repeated with `Q38_DENSE_GPU=1`. If `dn-proj` moves toward 19.6 the attenuation is clocks and the levers are the DPM profile (a rig setting — with its idle-watt cost on a box that serves 24/7, the owner's call) or a keepalive dispatch that holds dev0 awake across the gap; if it does not move, the attenuation is something else and Q7 steps 2–3's replayed predictions stand as the ceiling | `dn-proj` **26.98 in-engine vs 18.6–19.6 isolated** — ~7.4 ms/token of attenuation on step 1; ~2.5 more on step 2 at the same ratio; unmeasured on `vk-take` | §Q7's duty-cycle table; §Q3's finding that the gap is unevenly distributed across layers (a keepalive costs most in the layers that are already CPU-long) | **−5 to −10 ms/token** if it is clocks; 0 if it is not — step 0 says which, and the gate is derived from it | step 0 an hour (with sudo); a keepalive dispatch a day | Sonnet (step 0); Opus (the keepalive) | **bit-identical** (a DPM level or an empty dispatch changes no arithmetic); `[OPTIME] dn-proj`, `vk-dense`, `vk-take` and the token, three fresh-process repeats; idle power draw reported beside it; serving A/B with its own pristine rows |
| **Q14** | **STEP 0 DONE — THE HYPOTHESIS IS REFUTED, AND IT IS NOT A CPU TWIN OF Q13 (2026-09-12, Opus; record §Q14 step 0; branch `perf/q14-cpu-wait-policy`, NOT merged, nothing shipped).** Both zero-code legs are **null**: at mask 7, three interleaved fresh-process repeats each, `OMP_WAIT_POLICY=active` moves the gate's triple by **+0.094 / +0.000 / +0.131** (`gr-read` / `shared` / `cpu-experts`) and `+GOMP_SPINCOUNT=infinite` by **−0.065 / −0.115 / +0.027**, against the **+6.35 / +2.23** the item exists to recover; the token is 120.469 → 121.418 / 120.681 with all three repeats overlapping. **Oracle 6 of 6 sha256-identical.** The null is a result because it is bounded by a **positive control the row did not ask for**: `OMP_WAIT_POLICY=passive` costs **+36.08 ms/token (+29.9 %)** and lands **entirely** in this item's buckets — `gr-read` **+16.68**, `shared` +6.27, `cpu-experts` +9.21, `router` +1.76 — while every GPU bucket is flat (`vk-dense` +0.075, `dn-proj` +0.191). **So the axis has 16.7 ms/token of authority over `gr-read` and the default is already collecting all of it**: park-after-300k-spins vs never-park = 0.09; park-immediately = +16.68. **And the idle-ramp family is refuted directly, not only by the null**: 2 Hz per-core `scaling_cur_freq` puts the working cores within **5 MHz of the 3 901 MHz boost ceiling for the whole decode window at mask 0, mask 3 AND mask 7** (idle floor 2 500; `passive` is the only row that drops, to 3 324–3 411), and mask 7 records **fewer C1 entries/s than mask 0** — the mask accused of idling 97 times a token idles least. **Expected −7 ms/token from a wait-policy default: 0. There is no one-line env change to ship and no knob was added.** **What step 0 did establish, on the post-Q10 binary at three masks in one session:** the anomaly reproduces (`gr-read` +5.68, `shared` +2.30 from mask 0 to mask 7); it is **proportional to each bucket's work, not a fixed per-region cost** (over 96 sites `gr-down` pays +26 us/site and `gr-mix` +1.9 us/site, and `gr-rms` — serial and L1-resident — is the one loop that barely moves, ×1.057 against the GEMVs' ×1.26–1.32); read as bandwidth it is **69.5 → 54.4 GB/s on `gr-down` and 74.0 → 56.8 on `gr-up`, a 21–23 % loss on loops that did not change**; and **neither eviction nor submit count survives on this binary's own numbers** (m0→m3 evicts 1 096 experts for +1.42 of `gr-read`; m3→m7 evicts 272 and adds ONE submit for **+4.25**). **The leading candidates are now memory-side interference (dev0 DMA + the amdgpu driver's per-submit kernel work contending for DRAM/IF) or stolen cycles on the SMT siblings — both proportional, both invisible to `scaling_cur_freq`, neither fixable by a knob.** One 15-min zero-code measurement separates them: `perf stat -e cycles,instructions,task-clock,cache-misses` on the mask 3 / mask 7 pair — same cycles over more wall time = stolen time; more cycles with backend stalls = memory. **Leg 3 (the fence spin) was NOT run** under the campaign's pre-committed stop rule (no signal in legs 1–2, and the frequency evidence says the waiting thread is not where the cost is) — and it needs **no code**, because `vk_fence_wait()` has always read **`COLI_VK_SPIN_US` (default 300)**; the row's "throwaway build-time change" premise is wrong and `q14_probe.sh` already carries `spin0` / `spin3000` / `spininf` as variants. **No serving A/B and no warm-identical row: there is no candidate to price.** Harness `tools/hot-expert/q14_probe.sh` (q13_probe.sh with the variable made environmental, so legs interleave by round), logs `~/bench/q14_c1.log` / `q14_c2.log`, binary `5a9a2495` with all three cards at `auto`, asserted. **One methodological finding for everyone else: at `auto`, dev0's dense buckets drift ~8 % between campaigns an hour apart** (`vk-dense` 33.806 → 36.524, the token 116.531 → 120.469 on the same binary and prompt) — the CPU triple reproduced within 0.4, so cross-campaign dev0 comparisons need pairing, as Q13's did. Original row follows. **The CPU-side twin of Q13: the engine's OTHER processor also idles 97 times a token now** (new, logged at Q7 steps 2-3, 2026-09-12, record §Q7 steps 2-3 "What the arm costs on the CPU side"). With the whole Q7 arm on dev0 the three moved buckets fall **84.70 -> 38.88 (-45.82)** but the token falls only **-43.12**, and the difference is not noise: `gr-read` **21.86 -> 28.21 (+6.35)** and `shared` **7.33 -> 9.56 (+2.23)** -- CPU BF16 GEMV loops this item does not touch, over weights it does not move, ~28% slower and **stable across three repeats** (27.959 / 28.208 / 28.424 against 21.835 / 21.858 / 22.375). It does NOT track the eviction (mask 2 drops 250 experts and `gr-read` is flat at +0.12; mask 3 drops 1 096 and it is +1.84) nor the submit count alone (masks 3 and 7 differ by ONE submit and by 4.5 ms of `gr-read`). It does track the time the token spends waiting on a fence: `vk-dense` 0 / 8.5 / 4.1 / 34.5 / 38.9 against `gr-read` +0.00 / +0.12 / +0.18 / +1.84 / +6.35 across masks off / 2 / 4 / 3 / 7. **Hypothesis with a correlation behind it, not a finding**: the CPU now idles in sub-millisecond bursts between OpenMP regions and pays a ramp on each, which is Q13's shape on the other processor -- and `vk_fence_wait` spinning 300 us before it blocks is the first thing to look at, with `OMP_WAIT_POLICY` / `GOMP_SPINCOUNT` and the governor next. | `gr-read` +6.35 and `shared` +2.23 ms/token at mask 7, three repeats, medians, record §Q7 steps 2-3 | **~7 ms/token of the Q7 arm's raw -45.8 comes back here**, i.e. ~5% of the post-Q7 token; if it is the wait policy it may be a one-line env default | step 0 is zero-code (re-run the mask 3 / mask 7 [OPTIME] pair with `OMP_WAIT_POLICY=active` and `GOMP_SPINCOUNT=infinite`, then with the fence spin lengthened); half a day if that moves it | Sonnet for step 0, Opus if it is the fence | knob off bit-identical; the `gr-read` / `shared` / `cpu-experts` triple before and after at mask 7, three repeats; the token and the warm-identical row | **sequenced after Q13** -- they may be one mechanism and Q13's step 0 (the DPM performance level) needs the owner's sudo anyway |
...
logged as **Q11** together with the `q38_gr_apply` parallelisation that was
expected to make it free. **Q11 is DONE and REJECTED (2026-09-12, record §Q11):
the write-back's own pragma costs +0.72, the two-pragma shape this paragraph
describes costs +0.47, and the best shape that exists wins 0.10 on a token whose
spread is 0.6. The "makes it free" claim is false — one OpenMP region costs
4.51 µs on this box and these 96 sites carry only 12.8 µs of work each, so
`gr-apply ≤ 0.10` is unreachable by any pragma. Nothing shipped; the −0.4 to
−0.5 below is withdrawn.**
...
format left to ship behind one. **Measured 2026-09-12 (record §Q4): the first
summation-order item did NOT land in the default-on class as written — 3 of
1 200 long-prompt `teacher_forcing` flips at cos 0.999998812, greedy identical,
0 of 30 short. Arbitrated (§Q4 `### Arbitration`): the default-on class has
never been exercised (§G12 and §G14 both shipped opt-in), and for a
same-products reassociation a TF flip is decisive only where it is not a
near-tie. The class is therefore refined to "`teacher_forcing` identical over
≥ 1k positions, OR every change a near-tie (pristine top-2 margin ≤ 0.10
logits, the two argmaxes swapped) with the routing trace identical" — the
refinement takes effect when Q4's measurement passes it, and Q7-gpu's BF16
`fmt` is measured against the same wording. The owner's confirm-or-veto on the
bar stands as written above; this is what it would be confirming.**
...
Q5 (0 to −2, counter only) and Q11 (−0.4 to −0.5 — **measured 2026-09-12 and
REJECTED at −0.10 with its gate unreachable, record §Q11**) the only
bit-identical rows left); then **the BF16 branch, which QP(b) made the only branch** (Q4) to
...
10. **Track Q** has its baseline (§Q-REBASE), its profile (§Q-PROFILE) and its
   ...
   **Q2 is merged (2026-09-12, record §Q2 — NO arbitration, and that is the
   ...
   half as Q11 with the `q38_gr_apply` pragma that was expected to make it free.
   **Q11 measured that in five arms on 2026-09-12 and REJECTED it: the pragma
   does not make it free, it costs +0.47 to +0.72, and `gr-apply <= 0.10` is
   unreachable because one OpenMP region is 4.51 us at 96 sites (record
   §Q11).** **All three
   ...
   bandwidth or a latency item) and **Q11** (half a day, −0.4 to −0.5 —
   **done and REJECTED at −0.10, record §Q11**). Then
   ...
   *Superseded pointer, kept so the change is visible:* "NEXT, as of
   ...
   is a ~22 % loss of achieved DRAM bandwidth on unchanged CPU GEMVs — a
   throughput effect, not an idle ramp, and nothing in Q14's row can be
   shipped. Its one recommended follow-up is a 15-minute `perf stat` of the
   mask 3 / mask 7 pair (memory stalls vs stolen cycles); the item's own
   remaining levers are gone. Q5 remains open but is
