...
probe on the decode track and GLM-5.3 does not survive int3 experts
(`teacher_forcing` diverges 13/42 and 16/1232, cos 0.973/0.878 — main roadmap
§4h, record §G15); no converter, shader or kernel was built, so there is
nothing here for PCAP to inherit. The PCAP row below is corrected from
"partial; tracked on the main roadmap" to dead, so a future session doesn't
read "partial" as still-open capacity headroom.
...
conversation, `accept_ui.sh` and the matrix all pass on plain P9 (§"P9 on the
user's path"): 9/9 continuations with `expect_reuse == engine_reuse`, first
token flat 5.0–6.1 s across ten turns, and the matrix's 2 000-token row proves
the mechanism — its turn 1 *reasoned* (15 tokens) and turn 2 still reused
5 448 = 5 433 + 15. A second session closed the CANCEL flake in parallel
(rev 18 of its own numbering, §"CANCEL retry"): 6/6 abandoned requests
cancelled, next request 6.6–8.7 s, against my plain-P9 control of 7.0/7.0/6.8.
**`GLM53_PREFIX_CKPT_MIN=1024` is in service** — a 170-token chat was storing
a checkpoint and evicting a ~4 400-token one, which the 05:00 canary did every
morning. Final state browser-verified at 04:41: `accept_ui` PASS, first token
3.8 s, all four `accept_live` checks green on the one binary carrying the
ledger, the cancel retry and the new minimum. **Next, and the only open item
on this table: P10, value-based checkpoint eviction.**
...
| **P10** ✅ | Value-based checkpoint eviction | The engine keeps 4 checkpoint slots and evicts by recency, so the most *recent* capture survives and the most *valuable* one need not. Measured twice: a 170-token API chat stored ~160 MB and knocked out a 4 307-token capture inside the P9 gate (~24 min lost), and the 05:00 canary reproduced it nightly. `GLM53_PREFIX_CKPT_MIN=1024` (in service 2026-09-10) is the threshold half of the fix and it is blunt: it also refuses to cache a legitimate 500-token prefix. **A checkpoint's worth is length × how often it is hit, not recency** — the tool-block capture must survive a long one-off paste, which a length-only rule does not guarantee (that paste is longer). Keep a hit counter per slot, evict on the lowest `len × hits`, and let the threshold drop back toward its 128 default once eviction is no longer blind | a new conversation with the tool block keeps starting in ~3–5 s no matter what else the box has served since; the 500-token case is cacheable again | 1 day | Opus | a small-prefix flood (20 short chats) must not evict the tool-block capture: `CKPT hit prefix≥4300` still fires on the next UI chat, `accept_live.sh` PASS, and `ui_matrix.sh` turn-2 rows unchanged |
| **P11** ✅ | **The checkpoint-touch guard** — `perf/p11-ckpt-touch-guard`, **gated and in service 2026-09-10** (merge `7d0b8cf`) | `ckpt_disk_touch` located the file by size alone (`ftell(f) >= off`, `off` from the IN-MEMORY slot, header never re-read), so a four-byte counter could land inside a stranger's capture and every load-time check would still pass — `ckpt_restore` compares span byte totals, never content, so the KV came back silently wrong. Found by a code review the owner asked for, not by a failing gate. | `p11_chain.sh` exits 0 on all four steps against pristine `0cbf48a5` (P10): `prefill_gate` teacher_forcing PASS, logits PASS, speedup PASS (bit-identical, as a disk-side-only change must be), and the guard's five-case regression test — case C (a foreign capture at this path) FAILS on the pre-P11 binary and passes after, verified on both builds rather than asserted |
| **P12** ✅ | **Deterministic tool-call ids + the ledger records them** — `perf/p12-ledger-toolcall-sig` (`da5f530`, `9e12c6e`), merged `9ef6ff4` **2026-09-13** | A live Open WebUI conversation diverged on EVERY turn carrying a tool call, forcing a full re-prefill of the whole history each time — **63–65 minutes of unwanted wait per turn, repeatedly, on the owner's daily gateway.** Cause: the raw text of one generation is re-parsed for tool calls at several independent points (the API response, P8's reply-pin, P9's ledger) and `uuid.uuid4()` produced a different id at each parse — not only between requests, *within* one — while P9's ledger compares two parses for equality. | Ids now derive from `sha1(index, name, arguments)`, so every independent parse of the same raw text agrees; P12b additionally records a turn's actual `tool_calls` instead of discarding them. Both bugs were in the gateway's Python, no engine change |
| **PCAP ✗** | Capacity (cross-ref; this row used to be called P8) | int3 experts (`ROADMAP-2026-09.md` 4h / G15) — **DEAD 2026-09-11**: the numerics probe (int3 simulated on GLM-5.3's own int4 weights, CPU-only, no converter/shader/kernel built) failed before any conversion — `teacher_forcing` diverged 13/42 and 16/1232 positions, `last_logits` cos 0.973/0.878, greedy text differs (main roadmap §4h, record §G15) | none collected — the gate failed first | — | Opus | met, as a kill: greedy identity + `teacher_forcing` **FAILED** |
