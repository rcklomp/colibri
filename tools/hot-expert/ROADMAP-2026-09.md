...
| G12 | **REOPENED 09-05 at the second Fable gate — spec written: `G12-KDA-GPU-SPEC-2026-09-05.md`.** The KDA recurrence (decay, step, per-head norm) moves onto dev0 so one layer's eight projections, the recurrence and `ko` record into ONE submit where the engine pays two. Reverses `G4-KDA-SPEC` on that spec's own revisit condition, which §RP1-CORRECTION made true for the first time: items 1–6 done/closed and KDA the largest bucket (53.3 ms/token). Only the recurrence shader is new — the chained-submit pattern is `coli_vk_attn_qprep`, ported. **Not bit-identical** (GLSL `exp`, recurrent state): ships behind `COLI_KDA_GPU=1`, off by default, with a 1000-step shader-only drift test and long-horizon greedy identity as the oracle. GPU-resident state is re-synced to the CPU spans only at segment export/import. The earlier blocked reasoning stands as written in record §G12; it was correct that this needs a kernel | §RP1-CORRECTION: KDA 53.3 ms/token, 32.9 %, largest; 35.0 of it GPU submits | **−15 to −22 ms/token** (one round trip −8.3, CPU recurrence −18.6, three dispatches +5–10). The roadmap's earlier "up to −35" counted matvec compute that stays | 2–4 days, 3 commits | **Opus** (new kernel + non-identical oracle) | Stage 1: rel err ≤ 1e-5 over 1000 steps; Stage 2: greedy identical through 128 tokens on every bench prompt, cosine ≥ 0.99999 at 8/64/128; `[OPTIME] kda` 1.577 → **< 1.0 ms/call** go, > 1.2 stop |
