...
|---|---|---:|---|---|---|---|---|
| **Q3** | **BUILT 2026-09-11, bit-identical, GATE NARROWLY NOT MET — needs a Fable call before it merges** (`perf/q3-shared-expert-into-gap`, record §Q3). The reorder works exactly as described and is byte-identical on the logits at 32 and 128 tokens; it is the *size* that missed. `vk-take` **12.62 → 8.41 ms/token** (gate: ≤ 8.0), `moe` **55.90 → 51.75, −4.14** (gate: ≥ −5.0), the token **190.47 → 186.62 ms (−3.85)** against an expected −5 to −7; serving warm-identical +0.9%, rotating 4.045 → 4.115 mean (inside the harness's own spread). Measured cause, in the numbers and not tuned away: only **54% of the shared expert is absorbed**, because the gap's length varies per layer and the layers with a large CPU share had already spent theirs. The remaining ~3.4 ms/token of idle belongs to Q8 (shorter GPU side) or Q7-gpu (more GPU work), not to a reordering. **The call to make: merge it as a bit-identical −3.9 ms/token with the gate thresholds corrected in the record, or reject it.** Original row follows. **The shared expert into the GPU gap.** In `q38_moe_decode` the order today is router → shared expert (CPU, 8 threads) → issue the routed groups → CPU share in the gap → take. The shared expert is pure CPU work sitting **before** the issue, so none of it overlaps the GPU. Move the issue above it. §G9 on a different engine, and the highest ms-per-day on the board | vk-take **13.0 ms/token (6.8%) of pure idle wait**; shared expert **7.2 ms/token** on the wrong side of the issue | §G9 hid GLM's 22 ms/token of GPU wait inside CPU work that was already larger; here the CPU work exists and is simply ordered wrong. The gain is bounded by the smaller of the two figures, 7.2 | **−5 to −7 ms/token** | half a day | Sonnet | **bit-identical** (`ys` accumulation order unchanged — add the shared term last, as today); `[OPTIME] vk-take` ≤ 8 ms/token **and** `moe` down ≥ 5 ms/token |
...
has ~3.4 ms/token of idle in it (measured, record §Q3; this paragraph
originally estimated 5.8 from the full 7.2 being absorbed, and only 54% was),
while each expert moved off the GPU also
...
its counter into a token (see the §Q3 note).
...
estimated −5 to −7, record §Q3); then the int8 branch (Q7-cpu) to **~103–123**, or
...
10. **Track Q** has its baseline (§Q-REBASE), its profile (§Q-PROFILE) and its
   ...
   **Q3 is built and bit-identical but its gate is narrowly unmet
   (−3.85 ms/token against −5 to −7; record §Q3) — it is on
   `perf/q3-shared-expert-into-gap`, unmerged, awaiting the Fable call the
   roadmap reserves for exactly this case.** Q1 and Q2 come next either way,
